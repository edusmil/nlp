...
class DepInfo(object):
    ...
    def prepare_indices(self, lines,has_POS):        
        ...
        for line in lines:            
            ...
            achou_hifen = 0            
            texto = line [line.find("-") + 1 : line.find(",",line.find("-"))]
            postexto_hifen=line.find("-") + 1
            achou_hifen = texto.find("-")
            while achou_hifen >= 0:
                achou_hifen = texto.find("-")                
                postexto_hifen = postexto_hifen + achou_hifen + 1
                texto = line [postexto_hifen : line.find(",",line.find("-"))]
            
            p_idx = int(line [postexto_hifen : line.find(",",line.find("-"))] )#alterado para corrigir o hífen de portugues
            self.dep [idx]  = p_idx
...
class BiSNgrams(object):
    ...
    def store_all_DepNgrams(self, sentence, op):
        ...
        for item in self.DepNgrams:
            ...
            if any(ngram in s for s in dic):#Update the frequency of the ngram                                
                if ngram in dic:
    ...
    def get_next_combinations (self, value, children, sentence):
        ...
        #################### The first combination
            ...
            ################### The rest
            top = int(self.Combination (len(children), r))
